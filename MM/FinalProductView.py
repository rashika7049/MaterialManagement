...
def DisplayFinalProductByIdJSON(request):
    ...
    try:
        ...
        q = "select FP.*,(select C.categoryname from categories C where C.categoryid = FP.categoryid),(select S.subcategoryname from subcategory S where S.subcategoryid = FP.subcategoryid), (select P.productname from products P where P.productid = FP.productid) from finalproducts FP where finalproductid = {}".format(
            finalproductid)
        cmd.execute(q)
        row = cmd.fetchone()
        dbe.close()
...
def SaveEditFinalProductPicture(request):
    try:
        ...
        q = "update finalproducts set picture = '{}' where finalproductid = {}".format(filename, finalproductid)
        print(q)
        dbe, cmd = Pool.ConnectionPool()
        cmd.execute(q)
        dbe.commit()
